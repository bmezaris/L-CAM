...
This repository hosts the code and data lists for our two learning-based eXplainable AI (XAI) methods called L-CAM-Fm and L-CAM-Img, for deep convolutional neural networks that receive as input images (of dimensions W × H(na po oti einai MxM!)) and produce as output the image regions explaining the DCNN’s outcome(NA PO OTI EINAI CLASS SPECIFIC). Both methods use an attention mechanism (AM),  trained  end-to-end  along  with the original (frozen) DCNN, to derive class activation maps (CAMs)from the last convolutional layer’s feature maps (FMs). During training, CAMs are applied to the FMs (L-CAM-Fm) or the input image(L-CAM-Img) forcing the AM to learn the image regions explaining the DCNN’s outcome. Two widely used evaluation metrics, Increase in Confidence (IC) and Average Drop (AD), are used for evaluation.

- (8 montela kapou prepei na anfethei h loss function?!)This repository contains the code for training L-CAM, using VGG16 or ResNet-50 as the pre-trained backbone network along with the Attention Mechanism to derive visual explanations for the classification decisions of the VGG16 or ResNet-50 backbone.
- Instead of training, the user can also download the pre-trained models for L-CAM(again using VGG16 or ResNet-50 as the backbone network[here](https://drive.google.com/drive/folders/19Lyfqn7BQqEPTjvA40CJQQwJ8IU6KfI_?uspuspuspuspuspuspuspuspusp=sharing).
...
![alt text](https://github.com/gkartzoni/Aux-DCNN/blob/5b77eab26bc5161c00594616a38093d9bb6709b6/images/Heatmaps.png)
